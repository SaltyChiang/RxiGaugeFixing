...
void gRelax(ColorMatrix *tgf, ColorMatrix *grf, ColorMatrix *lf, int su2_index, int cb, int i, int xm, int ym, int zm, int tm, bool overrelax, double overrelaxParam)
{
    v[i] = tgf[i * Nd + 0] + tgf[xm * Nd + 0].adjoint();
    v[i] += tgf[i * Nd + 1] + tgf[ym * Nd + 1].adjoint();
    v[i] += tgf[i * Nd + 2] + tgf[zm * Nd + 2].adjoint();
    ...
    // v[i] += Eigen::dcomplex(0., 0.25) * grf[tm].adjoint() * lf[tm];
...
#pragma omp parallel for
    for (int i = 0; i < VOL; i++)
    {
        ...
        if ((t + z + y + x) % 2 == cb)
        {
            gRelax(tgf, grf, lf, su2_index, cb, i, xm, ym, zm, tm, overrelax, overrelaxParam);
            grf[i] = v[i] * grf[i];
        }
    }
}
